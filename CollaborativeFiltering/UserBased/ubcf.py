...
class Ubcf:
        ...
        def generate_submission_csv(self, test_path: str, output_path: str):
            ...
            test['rating'] = self.predictions[test['book_id'].map(self.item_to_index), test['user_id'].map(self.user_to_index)]
            test[['user_id', 'book_id', 'rating']].to_csv(output_path, index=False)
